...
- `TypeInvariant`是为了检查下面行为逻辑中修改的变量都是合法值。比如分布式锁`distLock`要么处于`Available`和`Expired`状态，并且由于此时没有客户端获取锁，`id`为`InvalidId`。或者已经有客户端获取了分布式锁，状态为`Acquired`，`id`为对应客户端标识符。
...
2. `C2`: 发送请求之后，客户端一直等待服务端回复。如果服务端回复`"granted"`，就代表自己已经成功获取分布式锁。
...
> 客户端处理请求和回复响应要放到多个标签的原因是，TLA+在同一个标签下的所有操作都是原子完成的，所以将这两个操作分别放到不同标签下。
>
